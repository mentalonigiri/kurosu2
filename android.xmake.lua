add_requires("libsdl", {configs = {shared = true, sdlmain = true}})
...
return table.concat(os.files(s), " ")
end
...
-- build simple java
	os.exec(f([[
javac -source 1.8 -target 1.8 -d build/obj 
-cp "${android_home}/platforms/android-${android_target_platform}/android.jar:src/main/java" 
-sourcepath ]]) .. "src/main/java " .. glob("src/main/java/org/libsdl/app/*.java") ..
" " .. glob("src/main/java/org/my/game/*.java"))
		os.exec(f([[
d8 --release
--min-api ${android_legacy_platform}
--lib "${android_home}/platforms/android-${android_target_platform}/android.jar" 
--output build/dex
	...
	--"build/obj/**/**/**/*.class")
	glob("build/obj/**/**/**/*.class"))
	os.cd("build/dex")
	os.run([[zip -r ../apk-unaligned.apk ]] .. glob("*"))
	os.cd("../..")
end
